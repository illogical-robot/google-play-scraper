...
const MAPPINGS = {
  ...
  genre: {
    ...
    fun: (dsData) => {
      ...
      let genre = [];
      if (R.is(Array, dsData[79])) {
        genre.push(...dsData[79].map(R.path([0, 0])).filter(function (el) {
          return (R.is(String, el) && el != null);
        }));
      }
      if (R.is(Array, dsData[118])) {
        genre.push(...dsData[118].map(R.path([0, 0, 0])).filter(function (el) {
          return (R.is(String, el) && el != null);
        }));
      }
  ...
  genreId: {
    ...
    fun: (dsData) => {
      ...
      let genreId = [];
      if (R.is(Array, dsData[79])) {
        genreId.push(...dsData[79].map(R.path([0, 2])).filter(function (el) {
          return (R.is(String, el) && el != null);
        }));
      }
      if (R.is(Array, dsData[118])) {
        genreId.push(...dsData[118].map(R.path([0, 0, 2])).filter(function (el) {
          return (R.is(String, el) && el != null);
        }));
      }
